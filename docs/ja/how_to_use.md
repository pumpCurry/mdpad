...
1. リリースページから最新の ZIP ファイル (`mdpad-x.x.x-win.zip`) をダウンロードします。
2. 任意のフォルダに展開します。
...
```bash
# リポジトリをクローン
git clone https://github.com/yourname/mdpad.git
cd mdpad
...
未保存の変更がある状態でウィンドウを閉じようとすると、保存確認ダイアログが表示されます。

**既存ファイルを編集中の場合:**

| ボタン | 動作 |
|---|---|
| **上書き保存する** | 元のファイルに上書き保存して終了 |
| **新規保存する** | 別のファイルとして保存して終了 |
| **保存せず終了する** | 変更を破棄して終了 |
